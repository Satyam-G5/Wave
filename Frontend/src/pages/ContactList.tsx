...
const ContactList: React.FC = () => {
  ...
  const callButton = (phoneid : any , name :any ) =>{
    console.log("Call Initiated with : ",name , phoneid);
    socketcontext?.setName(name);
    socketcontext?.setCallphone(phoneid);
  ...
  return (
    ...
    <h1 className='text-center text-3xl md:text-5xl p-10 font-serif font-semibold'>Contact List </h1>
    <div className="w-60vh md:ml-52 md:mr-52 grid grid-cols-1 gap-y-4 border-gradient">
      {contacts.map((contact: Contact) => (
        <div key={contact.socketId} className="flex items-center border-b border-gray-300 py-2">
          ...
          <button onClick={() => callButton(contact.phone_no , contact.firstname ) } className="bg-green-500 hover:text-black text-white flex flex-row px-4 py-2 rounded transition duration-300 ease-in-out transform hover:-translate-y+1 hover:scale-105">
